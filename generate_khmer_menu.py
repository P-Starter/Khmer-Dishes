...
SAMLOR_STYLES = [
    ...
    dict(key="korko", rom="Samlor Korko", km="សម្លការគោ", en="Khmer Country Stew",
         ...
         veg=[ing("green papaya", 200, "g", "veg", "cubed"),
              ...
              ing("eggplant", 3, "whole", "veg", "quartered"),
    ...
    dict(key="ktis", rom="Samlor Ktis", km="សម្លខ្ទិះ", en="Coconut Kroeung Soup",
         ...
         veg=[ing("eggplant", 3, "whole", "veg", "quartered"),
...
def iconic():
    ...
    out.append(dict(
        ...
        ingredients=[
            ...
            ing("palm sugar", 2, "tbsp", "seasoning"),
            ing("eggplant", 3, "whole", "veg", "diced"),
...
PHRASE_RULES = [
    ...
    # --- season/finish variants (high-frequency) ---
    (re.compile(r"^Season with oyster sauce, fish sauce and a pinch of palm sugar; toss quickly\.$"),
     lambda m: "បន្ថែមទឹកអយស្ទ័រ ទឹកត្រី និងស្ករត្នោតបន្តិច។ កូរលឿនៗ។"),
    (re.compile(r"^Season with fish sauce, palm sugar.*; toss to coat\.$"),
     lambda m: "បន្ថែមទឹកត្រី ស្ករត្នោត និងគ្រឿងផ្សំផ្សេងៗ។ កូរឱ្យសម។"),
    (re.compile(r"^Season with tamarind, fish sauce and palm sugar; balance to a clean sour-salty-sweet\.$"),
     lambda m: "បន្ថែមអំពិល ទឹកត្រី និងស្ករត្នោត។ ដាក់ឱ្យរសជាតិសម — ជូរ ប្រៃ ផ្អែម។"),
    (re.compile(r"^Season with soy, oyster and fish sauces; toss until smoky and fragrant\.$"),
     lambda m: "បន្ថែមទឹកសុីអ៊ីវ ទឹកអយស្ទ័រ និងទឹកត្រី។ កូរលើភ្លើងខ្លាំងរហូតមានក្លិនឈ្ងុយ។"),
    (re.compile(r"^Season with oyster sauce, dark soy and a pinch of sugar; toss over high heat until smoky\.$"),
     lambda m: "បន្ថែមទឹកអយស្ទ័រ ស៊ីអ៊ីវខ្មៅ និងស្ករបន្តិច។ កូរលើភ្លើងខ្លាំងរហូតមានក្លិនឈ្ងុយ។"),
    (re.compile(r"^Finish with sawtooth coriander and rice-paddy herb; serve with rice\.$"),
     lambda m: "ដាក់ជីអង្ការ និងម្អមនៅខាងលើ បរិភោគជាមួយបាយ។"),
    (re.compile(r"^Finish with shredded kaffir lime leaf and sliced chili\.$"),
     lambda m: "ដាក់ស្លឹកក្រូចសើចហាន់ និងម្ទេសហាន់នៅខាងលើ។"),
    (re.compile(r"^Finish with spring onion\. Serve with sliced cucumber, lime and chili sauce\.$"),
     lambda m: "ដាក់ស្លឹកខ្ទឹមបារាំងនៅខាងលើ។ បរិភោគជាមួយត្រសក់ហាន់ ក្រូចឆ្មារ និងទឹកម្ទេស។"),
    (re.compile(r"^Taste — it should sing of salty, sweet and sour\. Finish with (.+?)\.$"),
     lambda m: "ភ្លក់ — រសជាតិគួរតែឈ្ងុយ ប្រៃ ផ្អែម ជូរ។ ដាក់ " + m.group(1) + " នៅខាងលើ។"),
    (re.compile(r"^Taste and adjust, then finish with (.+?)\.$"),
     lambda m: "ភ្លក់ ហើយកែតម្រូវរសជាតិ បន្ទាប់មកដាក់ " + m.group(1) + "។"),

    # --- salad (nhoam/pleah) builder ---
    (re.compile(r"^Whisk the dressing — lime juice, fish sauce, palm sugar, garlic and chili — until the sugar dissolves\.$"),
     lambda m: "កូរទឹកជ្រលក់ — ទឹកក្រូចឆ្មារ ទឹកត្រី ស្ករត្នោត ខ្ទឹមស និងម្ទេស — រហូតស្កររលាយ។"),
    (re.compile(r"^Pour over the dressing and toss well just before serving\.$"),
     lambda m: "ស្នូរទឹកជ្រលក់ពីលើ ហើយលាយឱ្យស្មើ មុនពេលបរិភោគ។"),
    (re.compile(r"^Pile onto a plate and top with extra herbs, peanuts and fried shallots\.$"),
     lambda m: "ដាក់លើចាន ហើយរោយស្លឹក សណ្ដែកដី និងខ្ទឹមក្រហមចៀននៅខាងលើ។"),
    (re.compile(r"^Poach or grill the (.+?) \((.+?)\) until just cooked, then slice thinly\.$"),
     lambda m: "ស្ងោរ ឬ អាំង " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") រហូតទើបឆ្អិន បន្ទាប់មកហាន់ស្ដើង។"),
    (re.compile(r"^Slice the (.+?) very thinly and toss with half the lime juice; leave 10 minutes until it turns opaque\.$"),
     lambda m: "ហាន់ " + _km_prot(m.group(1)) + " ឱ្យស្ដើងៗ ហើយលាយជាមួយទឹកក្រូចឆ្មារពាក់កណ្ដាល; ទុក ១០ នាទីរហូតប្រែជាពណ៌ស។"),
    (re.compile(r"^Combine the (.+?) with (.+?)\.$"),
     lambda m: "លាយ " + _km_prot(m.group(1)) + " ជាមួយ " + m.group(2) + "។"),

    # --- kho (caramel braise) builder ---
    (re.compile(r"^Melt the palm sugar in a heavy pot until it caramelises to a deep amber\.$"),
     lambda m: "រលាយស្ករត្នោតក្នុងឆ្នាំងធ្ងន់ រហូតប្រែជាពណ៌ត្នោតចាស់។"),
    (re.compile(r"^Add the smashed garlic and the (.+?) \((.+?)\); turn to coat in the caramel\.$"),
     lambda m: "បន្ថែមខ្ទឹមសបុក និង " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ")។ លាយឱ្យសាច់ស្រូបនឹងស៊ីរ៉ូ។"),
    (re.compile(r"^Add fish sauce and the braising liquid, plus the other aromatics\.$"),
     lambda m: "បន្ថែមទឹកត្រី ទឹកស្ល និងគ្រឿងផ្សំផ្សេងៗ។"),
    (re.compile(r"^Cover and braise gently until (.+?) and the sauce is syrupy, about (\d+) minutes\.$"),
     lambda m: "គ្របគម្រប ហើយដាំថ្នមៗរហូត " + _km_cook(m.group(1)) + " ហើយទឹកក្រាស់ ប្រហែល " + m.group(2) + " នាទី។"),
    (re.compile(r"^Uncover and reduce until glossy and clinging\.$"),
     lambda m: "បើកគម្រប ដាំបន្តរហូតទឹកក្រាស់ និងភ្លឺ។"),

    # --- fried (chien) builder ---
    (re.compile(r"^Heat oil to 170°C\. Fry the (.+?) (.+?) and crisp\.$"),
     lambda m: "កម្ដៅប្រេងដល់ ១៧០°C។ ចៀន " + _km_prot(m.group(1)) + " " + _km_cook(m.group(2)) + " ហើយក្រៀម។"),
    (re.compile(r"^Drain on a rack\.$"),
     lambda m: "ច្រោះលើដែកក្រាត។"),
    (re.compile(r"^Pat dry and dust lightly with seasoned rice flour\.$"),
     lambda m: "សម្ងួត ហើយប្រឡាក់ម្សៅអង្ករបន្តិច។"),
    (re.compile(r"^Pat dry; no batter needed\.$"),
     lambda m: "សម្ងួត; មិនត្រូវការម្សៅ។"),
    (re.compile(r"^Pat dry and dust with seasoned flour\.$"),
     lambda m: "សម្ងួត ហើយប្រឡាក់ម្សៅប្រឡាក់។"),
    (re.compile(r"^Coat in a light tempura-style batter and fry until crisp\.$"),
     lambda m: "ស្រោបនឹងម្សៅទើនពួរស្រាល ហើយចៀនរហូតក្រៀម។"),

    # --- noodle (mi cha / kuy teav cha) builder ---
    (re.compile(r"^Soak or loosen the noodles so they don't clump\.$"),
     lambda m: "ត្រាំ ឬ លាត់នូដឯុលឱ្យស្បែកមិនជាប់គ្នា។"),
    (re.compile(r"^Heat a wok screaming hot; fry the garlic, then the (.+?) (.+?)\.$"),
     lambda m: "ដុំខ្ទះឱ្យក្ដៅខ្លាំង។ ឆាខ្ទឹមស រួចបន្ថែម " + _km_prot(m.group(1)) + " " + _km_cook(m.group(2)) + "។"),
    (re.compile(r"^Push aside, scramble the egg, then add the noodles and Chinese broccoli\.$"),
     lambda m: "រុញទៅខាង រួចកូរពងមាន់ បន្ទាប់មកដាក់នូដឯុល និងស្ពៃខៀវ។"),
    (re.compile(r"^Add bean sprouts at the end, toss once and plate\. Top with herbs and fried garlic oil\.$"),
     lambda m: "ដាក់សណ្ដែកបណ្ដុះនៅចុង លាយម្ដង ហើយដាក់ចាន។ ដាក់ស្លឹក និងប្រេងខ្ទឹមចៀននៅខាងលើ។"),

    # --- noodle (kuy teav / soup) ---
    (re.compile(r"^Simmer the stock with dried seafood, rock sugar and a little fish sauce for 30 minutes; keep hot\.$"),
     lambda m: "ដាំទឹកស៊ុបជាមួយសុីហ្វូដស្ងួត ស្ករអំពៅ និងទឹកត្រីបន្តិច រយៈពេល ៣០ នាទី; ទុកក្ដៅ។"),
    (re.compile(r"^Cook the (.+?) \((.+?)\) in the broth until done; slice if needed\.$"),
     lambda m: "ដាំ " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ក្នុងទឹកស៊ុបឱ្យឆ្អិន; ហាន់បើចាំបាច់។"),
    (re.compile(r"^Blanch the noodles and bean sprouts; divide between bowls\.?$"),
     lambda m: "ស្ងោរនូដឯុល និងសណ្ដែកបណ្ដុះ; ចែកដាក់ចាន។"),
    (re.compile(r"^Top with the (.+?) and ladle over the hot broth\.$"),
     lambda m: "ដាក់ " + _km_prot(m.group(1)) + " ខាងលើ ហើយស្នូរទឹកស៊ុបក្ដៅពីលើ។"),
    (re.compile(r"^Finish with fried garlic oil, spring onion and coriander; serve with lime, chili and herbs on the side\.$"),
     lambda m: "ដាក់ប្រេងខ្ទឹមចៀន ស្លឹកខ្ទឹមបារាំង និងវ៉ាន់សុយ; បរិភោគជាមួយក្រូចឆ្មារ ម្ទេស និងស្លឹក។"),

    # --- noodle (num banh chok / gravy) ---
    (re.compile(r"^Pound the kroeung smooth and simmer in stock with strained prahok\.$"),
     lambda m: "បុកគ្រឿងឱ្យល្អិត ហើយដាំក្នុងទឹកស៊ុបជាមួយប្រហុកច្រោះ។"),
    (re.compile(r"^Add the flaked grilled fish \(and your chosen (.+?)\) and the coconut milk; simmer into a fragrant gravy\.$"),
     lambda m: "បន្ថែមត្រីអាំងបុក (និង " + _km_prot(m.group(1)) + " ដែលបានជ្រើស) និងទឹកដូង; ដាំឱ្យបានទឹកក្រាស់ឈ្ងុយ។"),
    (re.compile(r"^Season with fish sauce and palm sugar to a savoury, slightly sweet balance\.$"),
     lambda m: "បន្ថែមទឹកត្រី និងស្ករត្នោតឱ្យរសជាតិសម ឈ្ងុយផ្អែមបន្តិច។"),
    (re.compile(r"^Loosen the fresh rice vermicelli into nests in each bowl\.$"),
     lambda m: "លាត់នំបញ្ចុកស្រស់ជារាងសំបុកក្នុងចានម្នាក់ៗ។"),
    (re.compile(r"^Ladle the warm gravy over and pile high with raw banana blossom, cucumber, long beans and herbs\.$"),
     lambda m: "ស្នូរទឹកគ្រឿងក្ដៅពីលើ ហើយដាក់ត្រយោងចេក ត្រសក់ សណ្ដែកគួរ និងស្លឹកជាច្រើនពីលើ។"),

    # --- noodle (ka tieu / coconut) ---
    (re.compile(r"^Fry the red kroeung in coconut cream until the oil splits and it is fragrant\.$"),
     lambda m: "ឆាគ្រឿងក្រហមជាមួយខ្ទិះដូងរហូតប្រេងបែក និងឈ្ងុយ។"),
    (re.compile(r"^Add the (.+?) \((.+?)\), then the coconut milk and a little stock; simmer into a rich gravy\.$"),
     lambda m: "បន្ថែម " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") រួចទឹកដូង និងទឹកស៊ុបបន្តិច; ដាំឱ្យបានទឹកក្រាស់ឆ្ងាញ់។"),
    (re.compile(r"^Stir in ground peanuts and season with fish sauce and palm sugar\.$"),
     lambda m: "កូរសណ្ដែកដីបុក ហើយបន្ថែមទឹកត្រី និងស្ករត្នោត។"),
    (re.compile(r"^Blanch the vermicelli into bowls and ladle over the coconut gravy\.$"),
     lambda m: "ស្ងោរនំបញ្ចុកដាក់ចាន ហើយស្នូរទឹកដូងពីលើ។"),
    (re.compile(r"^Top with bean sprouts, herbs and fried garlic oil\.$"),
     lambda m: "ដាក់សណ្ដែកបណ្ដុះ ស្លឹក និងប្រេងខ្ទឹមចៀននៅខាងលើ។"),

    # --- grilled (aing/dot/chakak) builder ---
    (re.compile(r"^Light a charcoal grill and let it burn down to glowing embers \(or heat a grill pan\)\.$"),
     lambda m: "ដុំភ្លើងធ្យូងឱ្យស្ងាត់ និងក្ដៅ (ឬ ដុំខ្ទះអាំង)។"),
    (re.compile(r"^Coat the (.+?) \((.+?)\) in the marinade and rest at least 30 minutes \(overnight is better\)\.$"),
     lambda m: "ប្រឡាក់ " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ជាមួយទឹកប្រឡាក់ ហើយទុកយ៉ាងតិច ៣០ នាទី (ប្រសើរបើទុកមួយយប់)។"),
    (re.compile(r"^Grill the (.+?) over medium coals, turning and basting, (.+?)\.$"),
     lambda m: "អាំង " + _km_prot(m.group(1)) + " លើធ្យូងភ្លើងមធ្យម ត្រលប់ និងលាបទឹកប្រឡាក់ " + _km_cook(m.group(2)) + "។"),
    (re.compile(r"^Serve with steamed rice and fresh herbs and cucumber\.$"),
     lambda m: "បរិភោគជាមួយបាយក្ដៅ ស្លឹកស្រស់ និងត្រសក់។"),

    # --- steamed (chhnganh) builder ---
    (re.compile(r"^Arrange on a heatproof plate and scatter over the aromatics\.$"),
     lambda m: "តម្រៀបលើចានធន់នឹងភ្លើង ហើយរោយគ្រឿងផ្សំខាងលើ។"),
    (re.compile(r"^Steam over rapidly boiling water (.+?)\.$"),
     lambda m: "ចំហុយលើទឹកដាំខ្លាំង " + _km_cook(m.group(1)) + "។"),
    (re.compile(r"^Drizzle over the sauce, top with the fresh garnish, and serve with rice\.$"),
     lambda m: "ស្នូរទឹកជ្រលក់ពីលើ រួចដាក់គ្រឿងស្រស់ខាងលើ ហើយបរិភោគជាមួយបាយ។"),

    # --- curry (char_krohom / kari / saraman) builder ---
    (re.compile(r"^Fry the red kroeung in oil over medium-high heat until deeply fragrant and darkened\.$"),
     lambda m: "ឆាគ្រឿងក្រហមក្នុងប្រេងលើភ្លើងមធ្យម-ខ្លាំង រហូតឈ្ងុយខ្លាំង និងពណ៌ចាស់។"),
    (re.compile(r"^Add the (.+?) \((.+?)\) and stir-fry to coat in the paste\.$"),
     lambda m: "បន្ថែម " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ហើយឆាឱ្យសាច់ស្រូបនឹងគ្រឿង។"),
    (re.compile(r"^Add a splash of coconut cream and the long beans; stir-fry until the sauce clings, about (\d+) minutes\.$"),
     lambda m: "បន្ថែមខ្ទិះដូងបន្តិច និងសណ្ដែកគួរ; ឆាឱ្យទឹកស្ងួតស្រូបនឹងសាច់ ប្រហែល " + m.group(1) + " នាទី។"),
    (re.compile(r"^Fry the red kroeung in coconut cream until the oil splits and it smells fragrant\.$"),
     lambda m: "ឆាគ្រឿងក្រហមជាមួយខ្ទិះដូងរហូតប្រេងបែក និងឈ្ងុយ។"),
    (re.compile(r"^Stir in the ground roasted peanuts and toasted coconut\.$"),
     lambda m: "កូរសណ្ដែកដីលីង និងដូងលីង។"),
    (re.compile(r"^Add the (.+?) \((.+?)\) and seal in the paste\.$"),
     lambda m: "បន្ថែម " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ហើយចូលក្នុងគ្រឿង។"),
    (re.compile(r"^Pour in the coconut milk and add the potato; simmer gently\.$"),
     lambda m: "ស្នូរទឹកដូងចូល និងបន្ថែមដំឡូងបារាំង; ដាំថ្នមៗ។"),
    (re.compile(r"^Simmer (.+?) and the potato is soft, about (\d+) minutes\.$"),
     lambda m: "ដាំ " + _km_cook(m.group(1)) + " ហើយដំឡូងបារាំងទន់ ប្រហែល " + m.group(2) + " នាទី។"),
    (re.compile(r"^Season with fish sauce and palm sugar; it should be mild, nutty and rich\.$"),
     lambda m: "បន្ថែមទឹកត្រី និងស្ករត្នោត; រសជាតិគួរតែស្រាល មានរសសណ្ដែកដី និងឈ្ងុយ។"),
    (re.compile(r"^Fry the red kroeung and curry powder in coconut cream until aromatic and the oil splits\.$"),
     lambda m: "ឆាគ្រឿងក្រហម និងម្សៅការីជាមួយខ្ទិះដូងរហូតឈ្ងុយ និងប្រេងបែក។"),
    (re.compile(r"^Add the (.+?) \((.+?)\) and brown lightly\.$"),
     lambda m: "បន្ថែម " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ហើយឆាឱ្យលឿងបន្តិច។"),
    (re.compile(r"^Pour in coconut milk and enough water to cover; add sweet potato and onion\.$"),
     lambda m: "ស្នូរទឹកដូងចូល និងទឹកគ្រប់ជាប់សាច់; បន្ថែមដំឡូងជ្វា និងខ្ទឹមបារាំង។"),
    (re.compile(r"^Simmer until (.+?) and the vegetables are tender, about (\d+) minutes\.$"),
     lambda m: "ដាំរហូត " + _km_cook(m.group(1)) + " ហើយបន្លែទន់ ប្រហែល " + m.group(2) + " នាទី។"),
    (re.compile(r"^Serve hot with crusty baguette or fresh rice noodles \(num banh chok\)\.$"),
     lambda m: "បរិភោគក្ដៅៗជាមួយនំបុ័ងស្រួយ ឬ នំបញ្ចុកស្រស់។"),

    # --- amok ---
    (re.compile(r"^Pound the red kroeung to a very smooth paste — smoothness is everything in amok\.$"),
     lambda m: "បុកគ្រឿងក្រហមឱ្យល្អិតៗ — ភាពល្អិតគឺជាគន្លឹះនៃអាម៉ុក។"),
    (re.compile(r"^In a bowl, whisk the kroeung with coconut milk, egg, fish sauce and palm sugar until thick and creamy\.$"),
     lambda m: "ក្នុងចាន កូរគ្រឿងជាមួយទឹកដូង ពងមាន់ ទឹកត្រី និងស្ករត្នោតរហូតក្រាស់ និងរលោង។"),
    (re.compile(r"^Fold in the (.+?) until well coated\.$"),
     lambda m: "បញ្ចូល " + _km_prot(m.group(1)) + " ហើយលាយឱ្យសាច់ស្រូបនឹងគ្រឿង។"),
    (re.compile(r"^Line banana-leaf cups with a soft leaf, spoon in the mixture and top with coconut cream\.$"),
     lambda m: "តម្រៀបស្លឹកចេកក្នុងពែង ដាក់ល្បាយចូល ហើយដាក់ខ្ទិះដូងពីលើ។"),
    (re.compile(r"^Steam over medium heat 20–25 minutes until set like a soft custard\.$"),
     lambda m: "ចំហុយលើភ្លើងមធ្យម ២០–២៥ នាទីរហូតកកដូចសង់ខ្យាទន់។"),
    (re.compile(r"^Garnish with coconut cream and shredded kaffir lime leaf; serve with rice\.$"),
     lambda m: "ដាក់ខ្ទិះដូង និងស្លឹកក្រូចសើចហាន់ពីលើ; បរិភោគជាមួយបាយ។"),

    # --- prahok ---
    (re.compile(r"^Fry minced lemongrass and garlic, then stir in strained prahok liquid\.$"),
     lambda m: "ឆាស្លឹកគ្រៃកិន និងខ្ទឹមស រួចកូរទឹកប្រហុកច្រោះ។"),

    # --- fried rice (bai cha) ---
    (re.compile(r"^Use cold, day-old rice and break up any clumps\.$"),
     lambda m: "ប្រើបាយត្រជាក់ដែលឆ្អិនពីមួយថ្ងៃ ហើយលាត់ឱ្យលែងជាប់គ្នា។"),
    (re.compile(r"^Heat a wok very hot; fry the garlic, then the (.+?) (.+?)\.$"),
     lambda m: "ដុំខ្ទះឱ្យក្ដៅខ្លាំង; ឆាខ្ទឹមស រួចបន្ថែម " + _km_prot(m.group(1)) + " " + _km_cook(m.group(2)) + "។"),
    (re.compile(r"^Push aside and scramble the eggs\.$"),
     lambda m: "រុញទៅខាង ហើយកូរពងមាន់។"),
    (re.compile(r"^Add the rice and onion; toss over high heat to coat and heat through\.$"),
     lambda m: "បន្ថែមបាយ និងខ្ទឹមបារាំង; ឆាលើភ្លើងខ្លាំងឱ្យសាច់ស្រូប និងក្ដៅ។"),

    # --- kari powder / tao jiew aromatic ---
    (re.compile(r"^Fry the garlic and onion, then bloom the curry powder in the oil\.$"),
     lambda m: "ឆាខ្ទឹមស និងខ្ទឹមបារាំង រួចដាក់ម្សៅការីក្នុងប្រេងឱ្យឈ្ងុយ។"),
    (re.compile(r"^Add celery and beaten egg, tossing until the egg ribbons through\.$"),
     lambda m: "បន្ថែមអង្គាមភ្នំ និងពងមាន់វាយ កូរឱ្យពងមាន់សន្លឹក។"),
    (re.compile(r"^Fry the garlic and ginger, then stir in the fermented soybean paste\.$"),
     lambda m: "ឆាខ្ទឹមស និងខ្ញី រួចកូរតៅជ្យូ។"),
    (re.compile(r"^Add sliced chili and big lengths of spring onion\.$"),
     lambda m: "បន្ថែមម្ទេសហាន់ និងស្លឹកខ្ទឹមបារាំងកាត់ជាដុំធំ។"),

    # --- drinks (smoothie/shake) ---
    (re.compile(r"^Add the (.+?), condensed milk, ice and a little syrup to a blender\.$"),
     lambda m: "ដាក់ " + m.group(1) + " ទឹកដោះគោផ្អែម ទឹកកក និងស៊ីរ៉ូបន្តិចចូលម៉ាសុីនកិន។"),
    (re.compile(r"^Blend until smooth and frothy; taste and adjust sweetness with a pinch of salt\.$"),
     lambda m: "កិនរហូតរលោង និងព្រួស; ភ្លក់ ហើយកែតម្រូវភាពផ្អែមដោយអំបិលបន្តិច។"),
    (re.compile(r"^Pour into a tall glass and serve immediately over ice\.$"),
     lambda m: "ស្នូរចូលកែវវែង ហើយបរិភោគភ្លាមៗលើទឹកកក។"),

    # --- desserts (sticky rice / sweet soup) ---
    (re.compile(r"^Steam the soaked sticky rice over high heat 25–30 minutes until tender\.$"),
     lambda m: "ចំហុយបាយដំណើបត្រាំលើភ្លើងខ្លាំង ២៥–៣០ នាទីរហូតទន់។"),
    (re.compile(r"^Warm the coconut milk with palm sugar, salt and pandan until the sugar dissolves \(do not boil\)\.$"),
     lambda m: "កម្ដៅទឹកដូងជាមួយស្ករត្នោត អំបិល និងស្លឹកតើយរហូតស្កររលាយ (កុំដាំ)។"),
    (re.compile(r"^Fold most of the sweet coconut milk through the hot rice and rest 15 minutes to absorb\.$"),
     lambda m: "លាយទឹកដូងផ្អែមភាគច្រើនជាមួយបាយក្ដៅ ហើយទុក ១៥ នាទីឱ្យជ្រាប។"),
    (re.compile(r"^Serve the sticky rice with the (.+?) and the reserved coconut sauce\.$"),
     lambda m: "បរិភោគបាយដំណើបជាមួយ " + _km_filling(m.group(1)) + " និងទឹកដូងដែលទុកសល់។"),
    (re.compile(r"^Simmer the (.+?) in water with pandan until soft\.$"),
     lambda m: "ដាំ " + _km_filling(m.group(1)) + " ក្នុងទឹកជាមួយស្លឹកតើយរហូតទន់។"),
    (re.compile(r"^Add palm sugar and a pinch of salt; stir until dissolved\.$"),
     lambda m: "បន្ថែមស្ករត្នោត និងអំបិលបន្តិច; កូររហូតស្កររលាយ។"),
    (re.compile(r"^Pour in the coconut milk and warm through gently without hard-boiling\.$"),
     lambda m: "ស្នូរទឹកដូងចូល កម្ដៅថ្នមៗ កុំឱ្យដាំខ្លាំង។"),
    (re.compile(r"^Serve warm or over ice as a sweet dessert soup \(bong-aem\)\.$"),
     lambda m: "បរិភោគក្ដៅ ឬ លើទឹកកកជាបបរផ្អែម (បង្អែម)។"),
    ...
    # --- soup helpers ---
    (re.compile(r"^Boil the kroeung in stock with toasted ground rice until aromatic\.$"),
     lambda m: "ដាំគ្រឿងក្នុងទឹកស៊ុបជាមួយម្សៅអង្ករលីងរហូតឈ្ងុយ។"),
    (re.compile(r"^Fry the kroeung in a little coconut cream, then pour in the rest of the coconut milk\.$"),
     lambda m: "ឆាគ្រឿងជាមួយខ្ទិះដូងបន្តិច រួចស្នូរទឹកដូងដែលនៅសល់។"),
    (re.compile(r"^Simmer stock with bruised lemongrass, galangal and kaffir lime leaf\.$"),
     lambda m: "ដាំទឹកស៊ុបជាមួយស្លឹកគ្រៃបុក រំដេង និងស្លឹកក្រូចសើច។"),
    (re.compile(r"^Bring stock to a boil with bruised lemongrass and galangal\.$"),
     lambda m: "ដាំទឹកស៊ុបជាមួយស្លឹកគ្រៃបុក និងរំដេង។"),

    # --- CHA aromatic step (style.arom) ---
    (re.compile(r"^Pound or blend the kroeung — lemongrass, galangal, turmeric, kaffir lime leaf, garlic and shallot — into a fragrant yellow paste\.$"),
     lambda m: "បុក ឬ កិនគ្រឿង — ស្លឹកគ្រៃ រំដេង រមៀត ស្លឹកក្រូចសើច ខ្ទឹមស និងខ្ទឹមក្រហម — ឱ្យបានគ្រឿងលឿងឈ្ងុយ។"),
    (re.compile(r"^Smash the garlic and shred a generous thumb of young ginger\.$"),
     lambda m: "បុកខ្ទឹមស ហើយចិតខ្ញីខ្ចីមួយមេដៃ។"),
    (re.compile(r"^Smash the garlic and rinse the fresh green peppercorn strands\.$"),
     lambda m: "បុកខ្ទឹមស ហើយលាងម្រេចខៀវកំពតស្រស់។"),
    (re.compile(r"^Pound the bird's-eye chili and garlic to a rough paste\.$"),
     lambda m: "បុកម្ទេសខ្មាំង និងខ្ទឹមសឱ្យបានគ្រឿងបុកសុះៗ។"),
    (re.compile(r"^Fry the garlic, then melt in the shrimp paste until aromatic\.$"),
     lambda m: "ឆាខ្ទឹមស រួចលាយកាពិឱ្យឈ្ងុយ។"),
    (re.compile(r"^Fry the minced garlic in hot oil until pale gold and fragrant\.$"),
     lambda m: "ឆាខ្ទឹមសកិនក្នុងប្រេងក្ដៅរហូតមានពណ៌មាស និងឈ្ងុយ។"),
    (re.compile(r"^Fry the garlic until fragrant\.$"),
     lambda m: "ឆាខ្ទឹមសឱ្យឈ្ងុយ។"),

    # --- CHA add step (style.add) ---
    (re.compile(r"^Toss in yardlong beans and sweet pepper and stir-fry until tender-crisp\.$"),
     lambda m: "បន្ថែមសណ្ដែកគួរ និងម្ទេសផ្អែម ហើយឆាឱ្យទន់និងក្រៀម។"),
    (re.compile(r"^Add sliced onion, spring onion and wood-ear mushroom; stir-fry briefly\.$"),
     lambda m: "បន្ថែមខ្ទឹមបារាំងហាន់ ស្លឹកខ្ទឹមបារាំង និងផ្សិតត្រចៀកជ្រូក; ឆាបន្តិច។"),
    (re.compile(r"^Add sliced chili and a handful of holy basil\.$"),
     lambda m: "បន្ថែមម្ទេសហាន់ និងស្លឹកជាក់មួយក្ដាប់។"),
    (re.compile(r"^Add long beans, then pile in holy basil at the end\.$"),
     lambda m: "បន្ថែមសណ្ដែកគួរ រួចដាក់ស្លឹកជាក់ច្រើននៅចុង។"),
    (re.compile(r"^Add eggplant and chili; stir-fry until the eggplant softens\.$"),
     lambda m: "បន្ថែមត្រប់ និងម្ទេស; ឆារហូតត្រប់ទន់។"),
    (re.compile(r"^Add big lengths of spring onion and sliced onion\.$"),
     lambda m: "បន្ថែមស្លឹកខ្ទឹមបារាំងកាត់ដុំធំ និងខ្ទឹមបារាំងហាន់។"),
    (re.compile(r"^Add sliced chili and pea eggplant\.$"),
     lambda m: "បន្ថែមម្ទេសហាន់ និងត្រប់ខ្ចី។"),
    (re.compile(r"^Add pineapple, tomato, cucumber and onion\.$"),
     lambda m: "បន្ថែមម្នាស់ ប៉េងប៉ោះ ត្រសក់ និងខ្ទឹមបារាំង។"),

    # --- samlor / machu vegetable ---
    (re.compile(r"^Add the (.+?) \((.+?)\) and simmer until almost cooked\.$"),
     lambda m: "បន្ថែម " + _km_prot(m.group(1)) + " (" + _km_prep(m.group(2)) + ") ហើយដាំរហូតជិតឆ្អិន។"),
    (re.compile(r"^Add the (.+?) and simmer until tender\.$"),
     lambda m: "បន្ថែម " + _km_veg(m.group(1)) + " ហើយដាំរហូតទន់។"),
    (re.compile(r"^Fry the red kroeung in a little coconut cream until the oil splits and it smells fragrant\.$"),
     lambda m: "ឆាគ្រឿងក្រហមជាមួយខ្ទិះដូងបន្តិចរហូតប្រេងបែក និងឈ្ងុយ។"),

    # --- iconic-ish remaining ---
    (re.compile(r"^Cook the pork and shrimp toppings in the broth\.$"),
     lambda m: "ដាំសាច់ជ្រូក និងបង្គាក្នុងទឹកស៊ុបឱ្យឆ្អិន។"),
    (re.compile(r"^Marinate sliced pork in lemongrass, garlic, fish sauce, palm sugar and honey\.$"),
     lambda m: "ប្រឡាក់សាច់ជ្រូកហាន់ជាមួយស្លឹកគ្រៃ ខ្ទឹមស ទឹកត្រី ស្ករត្នោត និងទឹកឃ្មុំ។"),

    # --- additional drinks / desserts ---
    (re.compile(r"^Brew strong coffee using a phin or dripper\.$"),
     lambda m: "ដាំកាហ្វេខ្លាំងដោយប្រើព្រន ឬ ឧបករណ៍ស្រក់។"),
    (re.compile(r"^Stir 2 tbsp condensed milk into the hot coffee\.$"),
     lambda m: "កូរទឹកដោះគោផ្អែម ២ ស្លាបព្រាចូលកាហ្វេក្ដៅ។"),
    (re.compile(r"^Pour over a glass of ice and stir well\.$"),
     lambda m: "ស្នូរលើកែវទឹកកក ហើយកូរឱ្យសម។"),
    (re.compile(r"^Press the sugarcane through a juicer\.$"),
     lambda m: "កិតអំពៅជាមួយម៉ាសុីនកិត។"),
    (re.compile(r"^Squeeze in a kumquat or lime for brightness\.$"),
     lambda m: "ច្របាច់កុមកាត់ ឬ ក្រូចឆ្មារដើម្បីភាពស្រស់។"),
    (re.compile(r"^Serve very cold over ice\.$"),
     lambda m: "បរិភោគត្រជាក់លើទឹកកក។"),
    (re.compile(r"^Brew strong black tea and cool\.$"),
     lambda m: "ដាំតែខ្មៅខ្លាំង ហើយត្រជាក់។"),
    (re.compile(r"^Sweeten with palm sugar syrup\.$"),
     lambda m: "ផ្អែមដោយស៊ីរ៉ូត្នោត។"),
    (re.compile(r"^Serve over ice with a squeeze of lime\.$"),
     lambda m: "បរិភោគលើទឹកកកជាមួយទឹកក្រូចឆ្មារ។"),
    (re.compile(r"^Open a chilled young coconut\.$"),
     lambda m: "បើកដូងខ្ចីត្រជាក់។"),
    (re.compile(r"^Pour the water over ice and scrape in the soft flesh\.$"),
     lambda m: "ស្នូរទឹកលើទឹកកក ហើយកោសសាច់ដូងទន់ចូល។"),
    (re.compile(r"^Serve immediately\.$"),
     lambda m: "បរិភោគភ្លាមៗ។"),
    (re.compile(r"^Squeeze fresh lime into a glass with syrup\.$"),
     lambda m: "ច្របាច់ក្រូចឆ្មារស្រស់ចូលកែវជាមួយស៊ីរ៉ូ។"),
    (re.compile(r"^Add a small pinch of salt\.$"),
     lambda m: "បន្ថែមអំបិលបន្តិច។"),
    (re.compile(r"^Top with soda water and ice\.$"),
     lambda m: "ដាក់ទឹកសូដា និងទឹកកកនៅខាងលើ។"),
    (re.compile(r"^Toast rice until deep golden and fragrant\.$"),
     lambda m: "លីងអង្ករឱ្យមានពណ៌មាសចាស់ និងឈ្ងុយ។"),
    (re.compile(r"^Simmer with pandan and a little rock sugar\.$"),
     lambda m: "ដាំជាមួយស្លឹកតើយ និងស្ករអំពៅបន្តិច។"),
    (re.compile(r"^Strain and serve hot or iced\.$"),
     lambda m: "ច្រោះ ហើយបរិភោគក្ដៅ ឬ ត្រជាក់។"),
    (re.compile(r"^Soak tamarind pulp in warm water and strain\.$"),
     lambda m: "ត្រាំសាច់អម្ពិលក្នុងទឹកក្ដៅ ហើយច្រោះ។"),
    (re.compile(r"^Sweeten and add a pinch of salt\.$"),
     lambda m: "ផ្អែម ហើយបន្ថែមអំបិលបន្តិច។"),
    (re.compile(r"^Serve cold over ice\.$"),
     lambda m: "បរិភោគត្រជាក់លើទឹកកក។"),
    (re.compile(r"^Blend soaked soybeans with water and strain\.$"),
     lambda m: "កិនសណ្ដែកសៀងត្រាំជាមួយទឹក ហើយច្រោះ។"),
    (re.compile(r"^Simmer with pandan, skimming foam\.$"),
     lambda m: "ដាំជាមួយស្លឹកតើយ ហើយដួសពពោះ។"),
    (re.compile(r"^Sweeten and serve hot or cold\.$"),
     lambda m: "ផ្អែម ហើយបរិភោគក្ដៅ ឬ ត្រជាក់។"),

    # --- snack one-offs ---
    (re.compile(r"^Split a crisp baguette and toast lightly\.$"),
     lambda m: "ពុះនំបុ័ងស្រួយ ហើយរំងាស់បន្តិច។"),
    (re.compile(r"^Spread with pâté and a swipe of butter\.$"),
     lambda m: "លាបប៉ាតេ និងបឺរបន្តិច។"),
    (re.compile(r"^Layer the sliced pork roll, pickled vegetables and cucumber\.$"),
     lambda m: "តម្រៀបសាច់ជ្រូកហាន់ បន្លែជ្រក់ និងត្រសក់។"),
    (re.compile(r"^Top with coriander, chili and a dash of soy; close and serve\.$"),
     lambda m: "ដាក់វ៉ាន់សុយ ម្ទេស និងទឹកសុីអ៊ីវបន្តិចនៅខាងលើ; បិទ ហើយបរិភោគ។"),
    (re.compile(r"^Marinate sliced pork in garlic, coconut water and a little sugar\.$"),
     lambda m: "ប្រឡាក់សាច់ជ្រូកហាន់ជាមួយខ្ទឹមស ទឹកដូងខ្ចី និងស្ករបន្តិច។"),
    (re.compile(r"^Grill over charcoal until caramelised\.$"),
     lambda m: "អាំងលើធ្យូងភ្លើងរហូតមានពណ៌ត្នោត។"),
    (re.compile(r"^Serve over warm broken rice with pickled vegetables\.$"),
     lambda m: "បរិភោគលើបាយចំការក្ដៅជាមួយបន្លែជ្រក់។"),
    (re.compile(r"^Add a small bowl of clear broth on the side\.$"),
     lambda m: "ដាក់ចានទឹកស៊ុបស្អាតមួយតូចនៅខាង។"),

    # --- samlor builder remaining ---
    (re.compile(r"^Add the vegetables \((.+?)\) and simmer until just tender\.$"),
     lambda m: "បន្ថែមបន្លែ (" + m.group(1) + ") ហើយដាំរហូតទើបទន់។"),
    (re.compile(r"^Bring stock to a boil and stir in the lemongrass kroeung\.$"),
     lambda m: "ដាំទឹកស៊ុប ហើយកូរគ្រឿងស្លឹកគ្រៃចូល។"),
    (re.compile(r"^Bring stock to a boil with smashed garlic and lemongrass\.$"),
     lambda m: "ដាំទឹកស៊ុបជាមួយខ្ទឹមសបុក និងស្លឹកគ្រៃ។"),

    # --- fry style finishes ---
    (re.compile(r"^Serve with sweet chili sauce or tuk trey p'aem\.$"),
     lambda m: "បរិភោគជាមួយទឹកម្ទេសផ្អែម ឬ ទឹកត្រីផ្អែម។"),
    (re.compile(r"^Toss the fried pieces in the caramel until glazed; finish with chili and spring onion\.$"),
     lambda m: "លាយសាច់ចៀនជាមួយស៊ីរ៉ូរហូតភ្លឺ; ដាក់ម្ទេស និងស្លឹកខ្ទឹមបារាំងនៅខាងលើ។"),
    (re.compile(r"^Pour the warm sweet-and-sour sauce over the fried pieces just before serving\.$"),
     lambda m: "ស្នូរទឹកជូរផ្អែមក្ដៅពីលើសាច់ចៀន មុនពេលបរិភោគ។"),
    (re.compile(r"^Crisp the ginger and garlic in the oil and scatter over the top with spring onion\.$"),
     lambda m: "ចៀនខ្ញី និងខ្ទឹមសក្នុងប្រេងឱ្យក្រៀម ហើយរោយលើជាមួយស្លឹកខ្ទឹមបារាំង។"),

    # --- grilled style finishes (aing/dot/chakak marinade & dip) ---
    (re.compile(r"^Blend lemongrass kroeung with fish sauce, palm sugar and a little oil into a marinade\.$"),
     lambda m: "កិនគ្រឿងស្លឹកគ្រៃជាមួយទឹកត្រី ស្ករត្នោត និងប្រេងបន្តិចជាទឹកប្រឡាក់។"),
    (re.compile(r"^Serve with tuk meric — lime juice, salt, black Kampot pepper and chili\.$"),
     lambda m: "បរិភោគជាមួយទឹកម្រេច — ទឹកក្រូចឆ្មារ អំបិល ម្រេចខ្មៅកំពត និងម្ទេស។"),
    (re.compile(r"^Rub generously with salt, smashed garlic and white pepper\.$"),
     lambda m: "ប្រឡាក់ដោយអំបិល ខ្ទឹមសបុក និងម្រេចស។"),
    (re.compile(r"^Serve with tuk trey p'aem — sweet-sour fish-sauce dip with chili and peanuts\.$"),
     lambda m: "បរិភោគជាមួយទឹកត្រីផ្អែម — ទឹកត្រីផ្អែមជូរជាមួយម្ទេស និងសណ្ដែកដី។"),
    (re.compile(r"^Marinate in honey, soy, garlic and lemongrass, then thread onto skewers\.$"),
     lambda m: "ប្រឡាក់ជាមួយទឹកឃ្មុំ ទឹកសុីអ៊ីវ ខ្ទឹមស និងស្លឹកគ្រៃ រួចចាក់ឈ្នាន់។"),
    (re.compile(r"^Serve with pickled vegetables and a chili-lime dip\.$"),
     lambda m: "បរិភោគជាមួយបន្លែជ្រក់ និងទឹកជ្រលក់ម្ទេសក្រូចឆ្មារ។"),

    # --- snack remaining steps ---
    (re.compile(r"^Dip rice paper briefly to soften\.$"),
     lambda m: "ត្រាំក្រដាសអង្ករបន្តិចឱ្យទន់។"),
    (re.compile(r"^Layer lettuce, herbs, vermicelli, pork and shrimp\.$"),
     lambda m: "តម្រៀបសាឡាត់ ស្លឹក មីសួ សាច់ជ្រូក និងបង្គា។"),
    (re.compile(r"^Roll tightly, folding in the sides\.$"),
     lambda m: "រុំឱ្យតឹង ហើយបត់ចំហៀង។"),
    (re.compile(r"^Serve with a peanut-hoisin dipping sauce\.$"),
     lambda m: "បរិភោគជាមួយទឹកជ្រលក់សណ្ដែកដី និងហោសុីន។"),
    (re.compile(r"^Mix the pork, soaked glass noodles, grated carrot and mushroom\.$"),
     lambda m: "លាយសាច់ជ្រូក មីសួត្រាំ ការ៉ុតកោស និងផ្សិត។"),
    (re.compile(r"^Roll tightly in wrappers, sealing with egg\.$"),
     lambda m: "រុំក្នុងស្បែករុំ ហើយបិទដោយពងមាន់។"),
    (re.compile(r"^Deep-fry until golden and crisp\.$"),
     lambda m: "ចៀននៅប្រេងជ្រៅរហូតមានពណ៌មាស និងក្រៀម។"),
    (re.compile(r"^Serve with lettuce, herbs and a sweet-sour dip\.$"),
     lambda m: "បរិភោគជាមួយសាឡាត់ ស្លឹក និងទឹកជ្រលក់ជូរផ្អែម។"),
    (re.compile(r"^Make a light batter with rice flour, sesame, a little sugar and coconut\.$"),
     lambda m: "ធ្វើម្សៅរលុងជាមួយម្សៅអង្ករ ល្ង ស្ករបន្តិច និងដូង។"),
    (re.compile(r"^Coat the bananas\.$"),
     lambda m: "ស្រោបចេក។"),
    (re.compile(r"^Deep-fry until deep golden and crisp\.$"),
     lambda m: "ចៀនជ្រៅរហូតមានពណ៌មាសចាស់ និងក្រៀម។"),
    (re.compile(r"^Drain and serve hot, optionally with palm-sugar syrup\.$"),
     lambda m: "ច្រោះ ហើយបរិភោគក្ដៅៗ ស្រេចចិត្តជាមួយស៊ីរ៉ូត្នោត។"),
    (re.compile(r"^Mix soaked sticky rice with beans, coconut milk and salt\.$"),
     lambda m: "លាយបាយដំណើបត្រាំជាមួយសណ្ដែក ទឹកដូង និងអំបិល។"),
    (re.compile(r"^Pack loosely into a bamboo tube lined with banana leaf\.$"),
     lambda m: "បំពេញក្នុងបំពង់ឫស្សីដែលរៀបស្លឹកចេក។"),
    (re.compile(r"^Roast over charcoal, turning, until cooked and fragrant\.$"),
     lambda m: "ដុតលើធ្យូងភ្លើង ត្រលប់ រហូតឆ្អិន និងឈ្ងុយ។"),
    (re.compile(r"^Peel back the bamboo and slice to serve\.$"),
     lambda m: "ដោះបំពង់ឫស្សី ហើយហាន់បរិភោគ។"),
    (re.compile(r"^Combine sliced banana with soaked sticky rice, coconut milk and sugar\.$"),
     lambda m: "លាយចេកហាន់ជាមួយបាយដំណើបត្រាំ ទឹកដូង និងស្ករ។"),
    (re.compile(r"^Wrap in banana leaf parcels\.$"),
     lambda m: "រុំក្នុងកញ្ចប់ស្លឹកចេក។"),
    (re.compile(r"^Steam 30–40 minutes until set\.$"),
     lambda m: "ចំហុយ ៣០–៤០ នាទីរហូតកក។"),
    (re.compile(r"^Cool slightly and serve\.$"),
     lambda m: "ត្រជាក់បន្តិច ហើយបរិភោគ។"),
    (re.compile(r"^Whisk eggs with palm sugar until dissolved\.$"),
     lambda m: "កូរពងមាន់ជាមួយស្ករត្នោតរហូតស្កររលាយ។"),
    (re.compile(r"^Stir in coconut milk and pandan extract\.$"),
     lambda m: "កូរទឹកដូង និងសារធាតុស្លឹកតើយចូល។"),
    (re.compile(r"^Strain into a dish or hollowed pumpkin\.$"),
     lambda m: "ច្រោះចូលចាន ឬ ផ្លែល្ពៅដែលហួស។"),
    (re.compile(r"^Steam gently until just set; chill before serving\.$"),
     lambda m: "ចំហុយថ្នមៗរហូតទើបកក; ត្រជាក់មុនពេលបរិភោគ។"),
    (re.compile(r"^Cut a lid in the pumpkin and scoop out the seeds\.$"),
     lambda m: "កាត់គម្របផ្លែល្ពៅ ហើយយកគ្រាប់ចេញ។"),
    (re.compile(r"^Whisk eggs, palm sugar, coconut milk and pandan; strain\.$"),
     lambda m: "កូរពងមាន់ ស្ករត្នោត ទឹកដូង និងស្លឹកតើយ; ច្រោះ។"),
    (re.compile(r"^Pour the custard into the pumpkin\.$"),
     lambda m: "ស្នូរសង់ខ្យាចូលក្នុងផ្លែល្ពៅ។"),
    (re.compile(r"^Steam 45–60 minutes until set; cool and slice into wedges\.$"),
     lambda m: "ចំហុយ ៤៥–៦០ នាទីរហូតកក; ត្រជាក់ ហើយកាត់ជាចំណិត។"),
    (re.compile(r"^Simmer rice in plenty of stock with ginger until broken down and creamy\.$"),
     lambda m: "ដាំបាយក្នុងទឹកស៊ុបច្រើនជាមួយខ្ញីរហូតបែក និងរលោង។"),
    (re.compile(r"^Season with fish sauce and white pepper\.$"),
     lambda m: "បន្ថែមទឹកត្រី និងម្រេចស។"),
    (re.compile(r"^Add cooked chicken or fish if using\.$"),
     lambda m: "បន្ថែមសាច់មាន់ ឬ ត្រីដាំស្រាប់ បើប្រើ។"),
    (re.compile(r"^Top with spring onion, fried garlic and herbs\.$"),
     lambda m: "ដាក់ស្លឹកខ្ទឹមបារាំង ខ្ទឹមសចៀន និងស្លឹកនៅខាងលើ។"),
    (re.compile(r"^Season soaked sticky rice with coconut and salt\.$"),
     lambda m: "ប្រឡាក់បាយដំណើបត្រាំជាមួយដូង និងអំបិល។"),
    (re.compile(r"^Wrap around a banana and beans in banana leaf into a log\.$"),
     lambda m: "រុំជុំវិញចេក និងសណ្ដែកក្នុងស្លឹកចេកជារាងបំពង់។"),
    (re.compile(r"^Tie firmly and boil several hours\.$"),
     lambda m: "ចងឱ្យតឹង ហើយដាំជាច្រើនម៉ោង។"),
    (re.compile(r"^Cool, unwrap and slice into rounds\.$"),
     lambda m: "ត្រជាក់ ដោះ ហើយហាន់ជារាងមូល។"),
    (re.compile(r"^Make a soft glutinous dough\.$"),
     lambda m: "ធ្វើម្សៅដំណើបទន់។"),
    (re.compile(r"^Fill with sweet coconut-palm-sugar paste\.$"),
     lambda m: "ដាក់សាច់ផ្អែមដូងស្ករត្នោតចូល។"),
    (re.compile(r"^Shape into pyramids and wrap in banana leaf\.$"),
     lambda m: "ធ្វើជារាងពីរ៉ាមីត ហើយរុំស្លឹកចេក។"),
    (re.compile(r"^Steam until cooked through\.$"),
     lambda m: "ចំហុយរហូតឆ្អិន។"),
    (re.compile(r"^Make a soft rice-flour batter and steam into rounds with chopped chives\.$"),
     lambda m: "ធ្វើម្សៅអង្ករទន់ ហើយចំហុយជារាងមូលជាមួយកុយឆាយកាត់។"),
    (re.compile(r"^Pan-fry until crisp and golden on both sides\.$"),
     lambda m: "ចៀនក្នុងខ្ទះរហូតក្រៀម និងមានពណ៌មាសសងខាង។"),
    (re.compile(r"^Serve hot with a sweet-sour fish-sauce dip\.$"),
     lambda m: "បរិភោគក្ដៅៗជាមួយទឹកត្រីផ្អែមជូរ។"),
    (re.compile(r"^Whisk rice flour, coconut milk, a little sugar and salt into a batter\.$"),
     lambda m: "កូរម្សៅអង្ករ ទឹកដូង ស្ករបន្តិច និងអំបិលជាម្សៅរលុង។"),
    (re.compile(r"^Heat a num krok pan and oil the wells\.$"),
     lambda m: "កម្ដៅខ្ទះនំគ្រក ហើយលាបប្រេងក្នុងរន្ធ។"),
    (re.compile(r"^Fill the wells and cook until the bottoms crisp and tops set\.$"),
     lambda m: "ដាក់ម្សៅចូលរន្ធ ហើយចៀនរហូតបាតក្រៀម និងលើកក។"),
    (re.compile(r"^Top with spring onion; serve warm in pairs\.$"),
     lambda m: "ដាក់ស្លឹកខ្ទឹមបារាំងនៅខាងលើ; បរិភោគក្ដៅៗជាគូ។"),

    # --- final SNACKS one-offs ---
    (re.compile(r"^Thread onto soaked skewers\.$"),
     lambda m: "ចាក់ឈ្នាន់ឫស្សីដែលត្រាំ។"),
    (re.compile(r"^Grill over charcoal, basting, until caramelised\.$"),
     lambda m: "អាំងលើធ្យូងភ្លើង លាបទឹកប្រឡាក់ រហូតមានពណ៌ត្នោត។"),
    (re.compile(r"^Marinate thinly sliced pork in garlic, coconut water and a little sugar\.$"),
     lambda m: "ប្រឡាក់សាច់ជ្រូកហាន់ស្ដើងជាមួយខ្ទឹមស ទឹកដូងខ្ចី និងស្ករបន្តិច។"),

    # --- catch-all: anything starting with "Serve with " not yet captured ---
    (re.compile(r"^Serve with (.+?)\.$"),
     lambda m: "បរិភោគជាមួយ " + m.group(1) + "។"),
]

# ----- ingredient / equipment / notes lookups (lowercase English -> Khmer) -----
KM_INGREDIENT = {
    # proteins
    "chicken": "សាច់មាន់", "pork": "សាច់ជ្រូក", "beef": "សាច់គោ",
    "duck": "សាច់ទា", "frog": "កង្កែប", "fish": "ត្រី",
    "shrimp": "បង្គា", "squid": "មឹក", "crab": "ក្ដាម", "clams": "ងាវ",
    "tofu": "តៅហ៊ូ", "egg": "ពងមាន់", "mushroom": "ផ្សិត",
    "mixed vegetables": "បន្លែ", "vegetable": "បន្លែ",
    "ground pork": "សាច់ជ្រូកកិន",
    "beef sirloin": "សាច់គោកំពូល",
    "beef shank & tripe": "សាច់គោជ្រ្ងាក និងពោះវៀន",
    "pork shoulder": "សាច់ស្មាជ្រូក",
    "pork bones": "ឆ្អឹងជ្រូក",
    "pork mince & sliced pork": "សាច់ជ្រូកកិន និងហាន់",
    "snakehead or catfish fillet": "សាច់ត្រី",
    "dried squid": "មឹកស្ងួត",
    "hard-boiled eggs": "ពងមាន់ស្ងោរ",
    "poached shrimp": "បង្គាស្ងោរ",
    # kroeung / aromatics
    "lemongrass": "ស្លឹកគ្រៃ", "galangal": "រំដេង",
    "fresh turmeric": "រមៀតស្រស់", "turmeric": "រមៀត",
    "kaffir lime leaves": "ស្លឹកក្រូចសើច", "kaffir lime leaf": "ស្លឹកក្រូចសើច",
    "garlic": "ខ្ទឹមស", "shallots": "ខ្ទឹមក្រហម", "shallot": "ខ្ទឹមក្រហម",
    "dried red chili": "ម្ទេសក្រហមស្ងួត", "fresh red chili": "ម្ទេសក្រហមស្រស់",
    "fresh chili": "ម្ទេសស្រស់", "bird's-eye chili": "ម្ទេសខ្មាំង",
    "fresh green kampot peppercorns": "ម្រេចខៀវកំពតស្រស់",
    "young ginger": "ខ្ញីខ្ចី", "ginger": "ខ្ញី",
    "kampot black pepper": "ម្រេចខ្មៅកំពត",
    "black pepper": "ម្រេចខ្មៅ", "white pepper": "ម្រេចស",
    "holy basil": "ស្លឹកជាក់", "basil": "ស្លឹកមៀន", "asian basil": "ស្លឹកមៀន",
    "coriander": "វ៉ាន់សុយ", "mint": "ស្លឹកសុណ្ដ",
    "sawtooth coriander": "ជីអង្ការ", "rice-paddy herb": "ម្អម",
    "spring onion": "ស្លឹកខ្ទឹមបារាំង", "onion": "ខ្ទឹមបារាំង",
    "garlic chives": "កុយឆាយ",
    "chinese celery": "អង្គាមភ្នំ",
    # coconut
    "coconut milk": "ទឹកដូង", "coconut cream": "ខ្ទិះដូង",
    "coconut water": "ទឹកដូងខ្ចី", "young coconut": "ដូងខ្ចី",
    "grated coconut": "ដូងកោសក", "toasted coconut": "ដូងលីង",
    # seasonings
    "fish sauce": "ទឹកត្រី", "fish sauce (teuk trey)": "ទឹកត្រី",
    "palm sugar": "ស្ករត្នោត", "sugar": "ស្ករ", "rock sugar": "ស្ករអំពៅ",
    "salt": "អំបិល", "a pinch of salt": "អំបិលបន្តិច",
    "vegetable oil": "ប្រេងបន្លែ", "oil for frying": "ប្រេងសម្រាប់ចៀន", "oil": "ប្រេង",
    "light soy sauce": "ទឹកសុីអ៊ីវស្រាល", "dark soy sauce": "ទឹកសុីអ៊ីវខ្មៅ",
    "soy sauce": "ទឹកសុីអ៊ីវ", "oyster sauce": "ទឹកអយស្ទ័រ",
    "tomato ketchup": "សុីអ៊ីវប៉េងប៉ោះ", "rice vinegar": "ទឹកខ្មេះអង្ករ",
    "prahok": "ប្រហុក", "prahok (fermented fish)": "ប្រហុក",
    "shrimp paste (kapi)": "កាពិ",
    "fermented soybean paste (tao jiew)": "តៅជ្យូ",
    "fermented black beans": "សណ្ដែកខ្មៅជ្រក់",
    "tamarind paste": "ទឹកអម្ពិល",
    "lime juice": "ទឹកក្រូចឆ្មារ", "lime": "ក្រូចឆ្មារ",
    "honey": "ទឹកឃ្មុំ", "pandan leaf": "ស្លឹកតើយ",
    "curry powder": "ម្សៅការី", "ground roasted peanuts": "សណ្ដែកដីបុក",
    "roasted peanuts": "សណ្ដែកដីលីង",
    "ground rice": "ម្សៅអង្ករ", "toasted ground rice": "ម្សៅអង្ករលីង",
    "evaporated milk": "ទឹកដោះគោកំហាប់",
    "sweetened condensed milk": "ទឹកដោះគោផ្អែម",
    "star anise": "ផ្កាសុរ៉ាឌី",
    # vegetables (most also in VEG_PAIRS km)
    "morning glory (water spinach)": "ត្រកួន", "morning glory": "ត្រកួន",
    "yardlong beans": "សណ្ដែកគួរ", "long beans": "សណ្ដែកគួរ",
    "bitter melon": "ម្រះ", "eggplant": "ត្រប់", "eggplant": "ត្រប់",
    "pea eggplant": "ត្រប់ខ្ចី",
    "cabbage": "ស្ពៃក្ដោប", "shredded cabbage": "ស្ពៃក្ដោបកាត់",
    "chinese broccoli (gai lan)": "ស្ពៃខៀវ", "chinese broccoli": "ស្ពៃខៀវ",
    "bamboo shoot": "ទំពាំង", "banana blossom": "ត្រយោងចេក",
    "pumpkin": "ល្ពៅ", "kabocha pumpkin": "ល្ពៅ",
    "bean sprouts": "សណ្ដែកបណ្ដុះ", "okra": "បែកពោះ", "water mimosa": "គ្នុង",
    "chayote": "ស៊ូ", "snake gourd": "នោងនោង", "ridge gourd": "រនោង",
    "winter melon": "ត្រសក់", "cauliflower": "ផ្កាស្ពៃ", "broccoli": "ផ្កាស្ពៃខៀវ",
    "choy sum": "ស្ពៃផ្កា", "napa cabbage": "ស្ពៃជ្រក់",
    "lotus stem": "ដើមឈូក", "lotus root": "មើមឈូក",
    "sweet potato leaves": "ស្លឹកដំឡូងជ្វា", "pumpkin vine tips": "ទំពាំងល្ពៅ",
    "mustard greens": "ស្ពៃខ្មៅ", "water lily stems": "ផ្ការំពើ",
    "green papaya": "ល្ហុង", "pineapple": "ម្នាស់",
    "tomato": "ប៉េងប៉ោះ", "cucumber": "ត្រសក់",
    "sweet potato": "ដំឡូងជ្វា", "potato": "ដំឡូងបារាំង", "daikon": "ហ៊ុនឆាយ",
    "moringa leaves": "ស្លឹកម្រុំ", "young tamarind leaves": "ស្លឹកអម្ពិលខ្ចី",
    "sour leaf (sorrel)": "ស្លឹកម្ជូរ",
    "carrot": "ការ៉ុត", "sweet pepper": "ម្ទេសផ្អែម",
    "wood-ear mushroom": "ផ្សិតត្រចៀកជ្រូក", "oyster mushroom": "ផ្សិតស្រូវ",
    "taro stem": "ត្រយ៉ូង", "taro": "ត្រាវ",
    "lettuce, tomato & onion": "សាឡាត់ ប៉េងប៉ោះ និងខ្ទឹមបារាំង",
    "raw vegetable platter": "ចានបន្លែស្រស់",
    # noodles & rice
    "flat rice noodles": "គុយទាវ", "wide rice noodles": "គុយទាវធំ",
    "fresh egg noodles": "មីពងមាន់ស្រស់", "rice vermicelli": "មីសួ",
    "fresh rice vermicelli (num banh chok)": "នំបញ្ចុក",
    "fine rice vermicelli (banh hoy)": "បាញ់ហយ",
    "jasmine rice": "អង្ករម្លិះ", "steamed jasmine rice": "បាយម្លិះ",
    "day-old cooked jasmine rice": "បាយម្លិះកាល់ពីមួយថ្ងៃ",
    "broken rice": "បាយចំការ",
    "glutinous (sticky) rice": "បាយដំណើប", "sticky rice": "បាយដំណើប",
    "rice flour": "ម្សៅអង្ករ", "glutinous rice flour": "ម្សៅអង្ករដំណើប",
    "cornstarch": "ម្សៅពោត", "tapioca starch": "ម្សៅដំឡូងមី", "tapioca pearls": "សាគូ",
    "sesame seeds": "ល្ង",
    # other
    "baguette": "នំបុ័ង", "crusty baguette": "នំបុ័ងស្រួយ",
    "noni or young cabbage leaves": "ស្លឹកនុ ឬ ស្លឹកស្ពៃខ្ចី",
    "banana leaf cups": "ពែងស្លឹកចេក", "banana leaf": "ស្លឹកចេក",
    "banana leaf parcels": "កញ្ចប់ស្លឹកចេក",
    "ripe banana": "ចេកទុំ", "ripe bananas": "ចេកទុំ",
    "black beans": "សណ្ដែកខ្មៅ", "black-eyed beans": "សណ្ដែកខ្មៅ", "red beans": "សណ្ដែកក្រហម",
    "mung beans": "សណ្ដែកបាយ", "soybeans": "សណ្ដែកសៀង",
    "lotus seed": "គ្រាប់ឈូក", "longan": "មៀន",
    "clear broth": "ទឹកស៊ុបស្អាត",
    "stock or water": "ទឹកស៊ុប ឬ ទឹក",
    "pork or chicken stock": "ទឹកស៊ុបជ្រូក ឬ មាន់",
    "fried garlic oil": "ប្រេងខ្ទឹមចៀន",
    "fried shallots": "ខ្ទឹមក្រហមចៀន",
    "fried garlic": "ខ្ទឹមសចៀន",
    "spring onion, coriander & lime": "ស្លឹកខ្ទឹមបារាំង វ៉ាន់សុយ និងក្រូចឆ្មារ",
    "spring onion & coriander": "ស្លឹកខ្ទឹមបារាំង និងវ៉ាន់សុយ",
    "water mimosa & long beans": "គ្នុង និងសណ្ដែកគួរ",
    "rice-paddy herb & sawtooth coriander": "ម្អម និងជីអង្ការ",
    "sawtooth coriander & rice-paddy herb": "ជីអង្ការ និងម្អម",
    "bamboo skewers": "ឈ្នាន់ឫស្សី",
    "bamboo tube": "បំពង់ឫស្សី",
    "lettuce & herbs": "សាឡាត់ និងស្លឹក",
    "mint & coriander": "ស្លឹកសុណ្ដ និងវ៉ាន់សុយ",
    "asian basil & mint": "ស្លឹកមៀន និងស្លឹកសុណ្ដ",
    "pickled carrot & daikon": "ការ៉ុត និងហ៊ុនឆាយជ្រក់",
    "pickled sour plum": "ផ្លែព្រូនជ្រក់",
    # dessert/drink one-offs
    "tamarind pulp": "សាច់អម្ពិល",
    "soy milk": "ទឹកសណ្ដែក", "sugar syrup": "ស៊ីរ៉ូស្ករ",
    "sugarcane": "អំពៅ", "fresh sugarcane": "អំពៅស្រស់",
    "kumquat or lime": "កុមកាត់ ឬ ក្រូចឆ្មារ",
    "dark-roast coffee": "កាហ្វេលីង", "black tea": "តែខ្មៅ",
    "palm sugar syrup": "ស៊ីរ៉ូត្នោត",
    "roasted rice": "អង្ករលីង",
    "crushed ice": "ទឹកកកបុក", "ice": "ទឹកកក",
    "noni or cabbage leaf": "ស្លឹកនុ ឬ ស្ពៃ",
    "green mango": "ស្វាយខ្ចី", "pomelo": "ក្រូចថ្លុង",
    "dried squid or shrimp": "មឹកស្ងួត ឬ បង្គា",
    "grilled fish": "ត្រីអាំង",
    "banana blossom, cucumber, long beans & herbs": "ត្រយោងចេក ត្រសក់ សណ្ដែកគួរ និងស្លឹក",
    "sawtooth coriander & mint": "ជីអង្ការ និងស្លឹកសុណ្ដ", "pandan": "ស្លឹកតើយ",
    "coconut": "ដូង", "sweet corn": "ពោតផ្អែម",
}

KM_NOTES = {
    "minced": "កិន",
    "sliced": "ហាន់", "thinly sliced": "ហាន់ស្ដើង",
    "finely sliced": "ហាន់ស្ដើង", "tender part, finely sliced": "យកផ្នែកទន់ ហាន់ស្ដើង",
    "smashed": "បុក", "bruised": "បុកបន្តិច",
    "chopped": "កាត់", "roughly chopped": "កាត់ដុំ", "cubed": "កាត់ជាគូប",
    "diced": "កាត់ជាគូបតូច",
    "cut into bite-sized pieces": "កាត់ជាដុំៗតូច",
    "cut into 2 cm cubes": "កាត់ជាគូប ២សម",
    "cut into chunks": "កាត់ជាដុំ", "chunks": "ដុំៗ",
    "cut in 4 cm lengths": "កាត់ ៤សម", "cut in 5 cm lengths": "កាត់ ៥សម",
    "cut in 6 cm lengths": "កាត់ ៦សម",
    "peeled and sliced": "ចេញសំបក ហើយហាន់",
    "peeled and cubed": "ចេញសំបក ហើយកាត់ជាគូប",
    "peeled and cut into wedges": "ចេញសំបក ហើយកាត់ជាចំណិត",
    "peeled, cut in 5 cm lengths": "ចេញសំបក ហើយកាត់ ៥សម",
    "peeled, sliced": "ចេញសំបក ហើយហាន់",
    "peeled and chopped": "ចេញសំបក ហើយកាត់",
    "peeled and deveined": "ចេញសំបក ហើយយកសរសៃ",
    "peeled": "ចេញសំបក",
    "julienned": "ហាន់ស្ដើងជាខ្សែ",
    "shredded": "កាត់ជាខ្សែស្ដើង",
    "shredded, soaked in lime water": "កាត់ខ្សែស្ដើង ហើយត្រាំទឹកក្រូចឆ្មារ",
    "shredded, drained": "កាត់ខ្សែស្ដើង ហើយច្រោះ",
    "de-stemmed": "ដកដើម",
    "soaked": "ត្រាំ",
    "soaked overnight": "ត្រាំមួយយប់",
    "soaked, optional for heat": "ត្រាំ (ស្ម័គ្រសម្រាប់ភាពហឹរ)",
    "soaked, for colour & heat": "ត្រាំ (សម្រាប់ពណ៌ និងភាពហឹរ)",
    "soaked, sliced": "ត្រាំ ហើយហាន់",
    "to serve": "សម្រាប់បរិភោគ",
    "mashed & strained": "បុក និងច្រោះ",
    "strained": "ច្រោះ",
    "quartered": "កាត់ជា ៤", "whole": "ទាំងមូល", "half": "ពាក់កណ្ដាល",
    "scrubbed": "សម្អាតសំបក", "cleaned and quartered": "សម្អាត ហើយកាត់ជា ៤",
    "halved lengthwise": "កាត់ពាក់កណ្ដាល",
    "wedges": "ចំណិត",
    "deseeded, sliced": "ដកគ្រាប់ ហើយហាន់",
    "or 1 thumb fresh": "ឬ ១មេដៃ ស្រស់",
    "minced & strained": "កិន និងច្រោះ",
    "mashed & strained": "បុក និងច្រោះ",
    "cleaned & jointed": "សម្អាត ហើយកាត់តាមសន្លាក់",
    "broken into florets": "បំបែកជាផ្កា",
    "soft leaves": "ស្លឹកទន់",
    "to line the cups": "សម្រាប់រៀបក្នុងពែង",
    "for the broth": "សម្រាប់ទឹកស៊ុប",
    "binds the mousse": "ភ្ជាប់សាច់",
    "or small bowls": "ឬ ចានតូច",
    "tender tips only": "យកតែចុងទន់",
    "or 1 thumb fresh": "ឬ ១មេដៃស្រស់",
    "minced": "កិន",
    "minced, optional": "កិន (ស្ម័គ្រ)",
    "minced and added at the end": "កិន ហើយដាក់នៅចុង",
    "very finely sliced": "ហាន់ស្ដើងបំផុត",
    "tender, drained": "ទន់ ច្រោះ",
    "boiled first": "ស្ងោរមុន",
    "sliced, boiled first": "ហាន់ ស្ងោរមុន",
    "soaked, sliced": "ត្រាំ ហើយហាន់",
    "soaked, drained": "ត្រាំ ហើយច្រោះ",
    "for the dip": "សម្រាប់ទឹកជ្រលក់",
    "for topping": "សម្រាប់ដាក់ពីលើ",
    "to taste": "តាមរសជាតិ",
    "added off the heat": "ដាក់ពេលដកពីភ្លើង",
    "key thickener": "ជាគ្រឿងក្រាស់សំខាន់",
    "coarsely cracked": "បុកសុះៗ",
    "segmented": "កាត់ចែកចំណិត",
    "cut into thin wedges": "កាត់ជាចំណិតស្ដើង",
    "cut into wedges": "កាត់ជាចំណិត",
    "cut into 4-cm lengths": "កាត់ ៤សម",
    "cut into 5-cm lengths": "កាត់ ៥សម",
    "cut into 6-cm lengths": "កាត់ ៦សម",
    "cut into pieces": "កាត់ជាដុំ",
    "cut into thick fillet slices": "ហាន់ជាបន្ទះក្រាស់",
    "cleaned and sliced": "សម្អាត ហើយហាន់",
    "cleaned and scored into rings": "សម្អាត ហើយហាន់ជាកង់",
    "trimmed": "កាត់ចុង",
    "knotted": "ចំរូង",
    "optional, for body": "ស្ម័គ្រ សម្រាប់រាងក្រាស់",
    "chilled": "ត្រជាក់",
}

KM_EQUIPMENT = {
    "wok": "ខ្ទះ",
    "mortar and pestle": "អង្គ្រែ",
    "soup pot": "ឆ្នាំងស៊ុប", "stock pot": "ឆ្នាំងស៊ុប",
    "pot": "ឆ្នាំង", "heavy pot": "ឆ្នាំងធ្ងន់",
    "pan": "ខ្ទះតូច",
    "steamer": "ឆ្នាំងចំហុយ",
    "banana leaf cups": "ពែងស្លឹកចេក",
    "charcoal grill": "ភ្លើងធ្យូង",
    "skewers": "ឈ្នាន់", "bamboo skewers": "ឈ្នាន់ឫស្សី",
    "mixing bowl": "ចានលាយ", "blender": "ម៉ាសុីនកិន",
    "wok or deep fryer": "ខ្ទះ ឬ ម៉ាសុីនចៀន",
}

# Bilingual section titles for the page (also exposed for any consumer).
KM_SECTION_TITLES = {
    "Ingredients": "គ្រឿងផ្សំ",
    "How to cook": "របៀបធ្វើ",
    "Equipment": "ឧបករណ៍",
    "Kroeung": "គ្រឿង",
}
        ...
        for pat, rep in PHRASE_RULES:
            ...
            if m:
                try:
                    ...
                    pass
                break
    # ingredients: item + notes Khmer where we know them.
    for ing_obj in record.get("ingredients", []):
        if "itemKm" not in ing_obj:
            km = KM_INGREDIENT.get((ing_obj.get("item") or "").lower())
            if km:
                ing_obj["itemKm"] = km
        if "notesKm" not in ing_obj and ing_obj.get("notes"):
            km = KM_NOTES.get(ing_obj["notes"].lower())
            if km:
                ing_obj["notesKm"] = km
    # equipment: parallel Khmer array, only if at least one item translates.
    eq = record.get("equipment") or []
    if eq and "equipmentKm" not in record:
        translated = [KM_EQUIPMENT.get(e.lower()) for e in eq]
        if any(translated):
            record["equipmentKm"] = [tr or en for tr, en in zip(translated, eq)]
